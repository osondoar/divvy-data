...
function load_data(evt, stationId){
  var stationToSelect = stationId || 286;
  var statuses = new Statuses(stationToSelect);
  var station = new Station({id: stationToSelect});
  var total_docks = 0;
  var chart_statuses = new Array;
  $.when(
  station.fetch(),
  statuses.fetch()
  ).then( 
